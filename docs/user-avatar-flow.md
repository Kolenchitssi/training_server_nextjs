...
### Шаг 6: Очистка аватара при удалении пользователя (`DELETE /api/user/:id`)

- При удалении пользователя из базы данных через `UserService.deleteUser` запрашивается поле `avatarPath`.
- Если у пользователя был привязан аватар, вызывается `filesService.deleteFile(avatarPath)` для удаления файла из хранилища `uploads/avatars/`, предотвращая появление сиротских файлов на диске.
